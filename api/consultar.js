    ...
    try {
        ...
        if (walletResponse.status === 403 || bankResponse.status === 403) {
            return res.status(403).json({ 
                error: 'Chave inválida ou sem permissão. Verifique as permissões de "account", "wallet" e "inventories".' 
            });
        }

        if (!walletResponse.ok || !bankResponse.ok) {
            return res.status(502).json({ error: 'Erro de comunicação com a API do GW2.' });
        }
        ...
        return res.status(200).json({ fractalRelics, pristineRelics, integratedMatrices });
    } catch (err) {
